...
typedef struct channel_t {
  ...
  pthread_mutex_t mutex; //Ha um mutex por canal para o programa correr concorrentemente
  pthread_cond_t     wait_for_free_space; //Por mutex ha 2 condicoes associadas
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  ...
  if(pthread_mutex_lock(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->mutex) != 0) {
  ...
  while (!mess) {
    if(pthread_cond_wait(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->wait_for_messages, &channel_array[tarefaDest*number_of_tasks+tarefaOrig]->mutex) != 0) {
  ...
  if(pthread_cond_broadcast(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->wait_for_free_space) != 0) {
    fprintf(stderr, "\nErro ao desbloquear variável de condição\n");
    return -1;
  }

  if(pthread_mutex_unlock(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->mutex) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->wait_for_free_space, &channel_array[tarefaDest*number_of_tasks+tarefaOrig]->mutex) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
  ...
  pthread_cond_broadcast(&channel_array[tarefaDest*number_of_tasks+tarefaOrig]->wait_for_messages);
