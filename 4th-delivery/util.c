...
/*--------------------------------------------------------------------
| Function: parse_char_or_exit
| Description: Processa a string str, do parametro name, como uma string
---------------------------------------------------------------------

double parse_char_or_exit(char const *str, char const *name) {
  char* string;
  if (sscanf(str, "%lf", &value) != 1) {
    fprintf(stderr, "\nErro no argumento \"%s\".\n\n", name);
    exit(-1);
