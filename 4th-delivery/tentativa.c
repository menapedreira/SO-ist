...
double dualBarrierWait (DualBarrierWithMax* b, int current, double localmax) {
  ...
  if (b->pending[current]==0) {
    ...
    /*flag que trata SIG_INT - cria processo filho*/
    if(flag==1){
    /*child process */
    if ((pid = fork()) == 0){
      fp = fopen(fichS,"w");
    ...
    /*flag alarme que trata SIGALRM*/
    if(alarme==1){
      wait(&status);
    /*child process*/
    if ((pid = fork()) == 0){
      signal(SIGINT,SIG_DFL); //processo de salvaguarda deve terminar
...
int main (int argc, char** argv) {
  ...
  tam_fatia = N / trab;
  terminado = 0;
  /*flag - handler escreve salvaguarda do sig_int*/
  flag=0;
  alarme=0;
