...
/*--------------------------------------------------------------------
| Function: writeMatrix2dOnFile
---------------------------------------------------------------------*/
void writeMatrix2dOnFile(FILE *f, DoubleMatrix2D *matrix) {
  int i, j;
  DoubleMatrix2D * m = (DoubleMatrix2D*) matrix;

  if (f == NULL || m == NULL)
    die("Erro de parametros");
  ...
  for (i = 0; i < m->n_l; i++) {
    for (j = 0; j < m->n_c; j++) {
      if (fprintf(f, " %8.4f", dm2dGetEntry(matrix, i, j)) < 0) {
        die("Erro ao escrever a matriz no ficheiro na salvaguarda.");
      }
    }
    if (fprintf(f, "\n" ) < 0)
      die("Erro ao fazer fprintf.");
  }
}
